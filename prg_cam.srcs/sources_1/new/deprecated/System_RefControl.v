...
//       It listens for frame completion events, selects the configured camera,
//       and programs a DMA MM2S transfer only when frame 0 of that camera is
//       available as the reference image.
//
// Notes:
...
//   or place an AXI interconnect in front of the Lite slaves.
// - This module is intended for the reference DMA path; gate it with a
//   reference camera id so it only reacts to frame 0 of that camera.
//
            ...
            case(state)
                IDLE: begin
                    ...
                    if (frame_done_pulse && Reference_req &&
                        (done_cam_id == REFERENCE_CAM_ID) && (done_frame_ptr == 4'd0)) begin
                        // The reference camera's first frame is ready; program the DMA.
