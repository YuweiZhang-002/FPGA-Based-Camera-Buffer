...
module Line_Generator #(
    ...
    input  wire        watchdog,      // Arbitration timeout error flag (same clk domain)
    ...
    // R2 fix: the shipping line/frame number is NO LONGER registered. It is exposed
    // combinationally from the buffer regs (see line_num_ext/frame_num_ext below),
    // removing the 1-cycle latch that made AXI sample the previous line's address.
    reg       is_transmitting; // Flag: a transaction is in progress for this LG.
    ...
    wire valid_pulse = cam_clk;

    wire normal_pixel_valid = valid_pulse && !in_overflow && (state == GET_LINE);
    assign overflow_en = valid_pulse && in_overflow && (state == GET_LINE);
    ...
    reg is_final_line;
    assign final_frame = is_final_line;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // Timeout abort (permanent error flag, SAME clk domain as this LG, so
            // no CDC). Mirror the AXI side: park the FSM and release the buffers,
            // otherwise this LG keeps is_transmitting/buf_full set forever (the
            // orphaned-buffer case). Held flag -> stays parked until global reset.
            line_num        <= 10'd0;
            pixel_num       <= 10'd0;
            in_overflow     <= 1'b0;
            ptr_rx          <= 1'b0;
            ...
            // Only mark ownership of the in-flight transaction; the address itself
            // is combinational now (R2 fix), so nothing needs latching here.
                ...
                WAIT_HREF: begin
                    ...
                    if (href) begin
                        ...
                        if(line_num == num_lines - 1) 
                            is_final_line <= 1'b1;        // Set the final_line flag, indicate a frame ready
    ...
    assign line_num_ext  = ptr_tx ? line_num_buf2 : line_num_buf1;
    assign frame_num_ext = ptr_tx ? frame_num_buf2 : frame_num_buf1;
